...
class Im_LivechatChannel(models.Model):
    ...
    # Channel Methods
    # --------------------------
    def _get_livechat_discuss_channel_vals(self, chatbot_script=None, operator=None, country_id=None):
        if not operator and not chatbot_script:
            return False
        operator_partner_id = (
        ...
        members_to_add = [
            Command.create(
                {
                    "livechat_member_type": "agent" if operator else "bot",
                    "chatbot_script_id": chatbot_script.id if not operator else False,
        ...
        if not self.env.user._is_public():
            ...
            if (
                ...
                members_to_add.append(
                    Command.create(
                        {"livechat_member_type": "visitor", "partner_id": visitor_user.partner_id.id}
                    )
                )
        if chatbot_script:
        ...
        return {
            ...
            "livechat_failure": "no_answer" if operator else "no_failure",
            'chatbot_current_step_id': chatbot_script._get_welcome_steps()[-1].id if chatbot_script else False,
            'anonymous_name': visitor_user.display_name if visitor_user else guest.name,
