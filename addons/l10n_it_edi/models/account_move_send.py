...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_invoice_extra_attachments(self, invoice):
        # EXTENDS 'account'
        return super()._get_invoice_extra_attachments(invoice) + self.env['ir.attachment'].sudo().search([
    ...
    def _link_invoice_documents(self, invoices_data):
        ...
        for move, data in invoices_data.items():
            if data.get('l10n_it_edi_values'):
                move.l10n_it_edi_attachment_file = base64.b64encode(data['l10n_it_edi_values']['raw'])
                move.l10n_it_edi_attachment_name = data['l10n_it_edi_values']['name']
                move.invalidate_recordset(fnames=['l10n_it_edi_attachment_name', 'l10n_it_edi_attachment_file'])
                moves_to_names[move.id] = data['l10n_it_edi_values']['name']

        if moves_to_names:
            attachments = self.env['ir.attachment'].sudo().search([
                ('res_model', '=', 'account.move'),
                ('res_field', '=', 'l10n_it_edi_attachment_file'),
                ('res_id', 'in', list(moves_to_names.keys())),
            ])
            for attachment in attachments:
                attachment.name = moves_to_names.get(attachment.res_id)
