...
class AccountMove(models.Model):
    ...
    def _l10n_ro_edi_fetch_invoices(self):
        ...
        # Non-indexed moves that were not prcessed after some time have probably been refused by the SPV. Since
        ...
        for invoice in non_indexed_invoices:
            sent_document = invoice.l10n_ro_edi_document_ids.sorted()[0]

            if (fields.Datetime.today() - sent_document.create_date).days > HOLDING_DAYS:
                document_ids_to_delete += invoice.l10n_ro_edi_document_ids.ids

                error_message = _(
                    "The invoice has been refused by the SPV. We were unable to recover the reason of the refusal because "
    ...
    def _l10n_ro_edi_process_invoice_accepted_messages(self, sent_invoices_accepted_messages):
        ''' Process the validation messages of invoices sent
            ...
            It will also attempts to recover the original invoices, that are missing their index,
            ...
            Send new 11 that has not yet been sent, it should gotten index AB but did not receive it.
        ...
        for message in sent_invoices_accepted_messages:
            ...
            if not invoice:
                # The move related to the message does not have an index, attempt to find one with the same name
                if 'error' in message or not (invoice := name_to_move.get(message['answer']['invoice']['name'])):
                    continue
            ...
            # Only delete invoice_sent documents and not all because one invoice can contain several signature for the reason stated
    ...
    def _l10n_ro_edi_process_bill_messages(self, received_bills_messages):
        ...
        for message in received_bills_messages:
            ...
            bill._extend_with_attachments(self.env['ir.attachment'].browse(xml_attachment_id))
