...
class AccountMove(models.Model):
    ...
    def _compute_l10n_tw_edi_is_b2b(self):
        for rec in self:
            rec.l10n_tw_edi_is_b2b = rec.partner_id.parent_id or rec.partner_id.company_type == "company"
    ...
    def _l10n_tw_edi_check_tax_type_on_invoice_lines(self):
        ...
        if product_lines.filtered(lambda line: not line.tax_ids or len(line.tax_ids) > 1):
            errors.append(self.env._("Invoive lines without taxes or more than one tax are not allowed."))

        if len(set(product_lines.tax_ids.mapped('price_include'))) > 1:
            errors.append(self.env._("Invoive lines with different tax include/exclude are not allowed."))
    ...
    def _l10n_tw_edi_check_before_generate_invoice_json(self):
        ...
        if self.partner_id.phone:
            if not self.partner_id.phone.startswith('+') or not re.match(r"\+\d{2,3} ", self.partner_id.phone):
    ...
    def _l10n_tw_edi_prepare_item_list(self, json_data, is_allowance=False):
        ...
        for line in self.invoice_line_ids.filtered(lambda line: line.display_type == "product"):
            ...
            if self.l10n_tw_edi_is_b2b:
                item_price = self._l10n_tw_edi_convert_currency_to_twd(round(line.price_subtotal / line.quantity))
                item_amount = self._l10n_tw_edi_convert_currency_to_twd(line.price_subtotal)
            else:
                item_price = self._l10n_tw_edi_convert_currency_to_twd(line.price_unit)
                item_amount = self._l10n_tw_edi_convert_currency_to_twd(line.price_total)

            if not self.l10n_tw_edi_is_b2b and is_allowance and not line.tax_ids[0].price_include:
                item_price = item_price * (1 + line.tax_ids[0].amount / 100)
            ...
            if self.l10n_tw_edi_is_b2b and is_allowance:
                item_list.append({
                    ...
                    "OriginalSequenceNumber": line.sequence,
                    "ItemName": line.product_id.name[:100],
                    "ItemCount": line.quantity,
                    "ItemPrice": item_price,
                    "ItemAmount": item_amount,
                    "Tax": round(item_amount * line.tax_ids[0].amount / 100)
                })
            else:
                item_list.append({
                    ...
                    "ItemPrice": item_price,
                    "ItemTaxType": line.tax_ids[0].l10n_tw_edi_tax_type if tax_type != "4" else "",
                    "ItemAmount": item_amount,
                })
        if self.l10n_tw_edi_is_b2b and is_allowance:
    ...
    def _l10n_tw_edi_generate_invoice_json(self):
        ...
        if self.l10n_tw_edi_is_b2b:
            tax_amount = 0 if tax_type == "4" else self.amount_tax
            amount_total = self.amount_untaxed if tax_type == "4" else self.amount_total
            json_data.update({
                "SalesAmount": self._l10n_tw_edi_convert_currency_to_twd(self.amount_untaxed),
                "TaxAmount": self._l10n_tw_edi_convert_currency_to_twd(tax_amount),
                "TotalAmount": self._l10n_tw_edi_convert_currency_to_twd(amount_total),
            })
        else:
            json_data.update({
                "CustomerName": self.partner_id.name,
                "Print": "1" if self.l10n_tw_edi_is_print else "0",
                ...
                "vat": "1" if self.invoice_line_ids.filtered(lambda line: line.display_type == "product")[0].tax_ids[0].price_include else "0",
            })
    ...
    def _l10n_tw_edi_generate_issue_allowance_json(self):
        ...
        if not self.l10n_tw_edi_is_b2b:
            ...
            json_data.update({
                ...
                "TaxAmount": self._l10n_tw_edi_convert_currency_to_twd(self.amount_tax),
                "TotalAmount": self._l10n_tw_edi_convert_currency_to_twd(self.amount_untaxed),
            })
