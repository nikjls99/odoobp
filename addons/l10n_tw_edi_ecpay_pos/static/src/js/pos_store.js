...
patch(PosStore.prototype, {
    async selectPartner() {
        ...
        if (this.company.country_id?.code === "TW" && this.config.is_ecpay_enabled) {
            ...
            if (this.get_order().get_partner().id === this.session._default_tw_customer_id) {
                posOrder.set_to_invoice(true);
            } else {
                posOrder.set_to_invoice(false);
            }
            posOrder.set_invoice_info(false, false, false, false);
        }
        return res;
    },
    ...
    async syncAllOrders(options = {}) {
        ...
        const posOrder = this.get_order();
        if (this.company.country_id?.code === "TW" && this.config.is_ecpay_enabled) {
            posOrder.l10n_tw_edi_is_b2b = Boolean(
                posOrder.partner_id.parent_id || posOrder.partner_id.company_type === "company"
            );
        }
        if (
            result &&
