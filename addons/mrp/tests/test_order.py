...
class TestMrpOrder(TestMrpCommon):
    ...
    def test_update_mo_from_bom_forecast(self):
        ...
        self.bom_1.write({
            'bom_line_ids': [
                Command.create({'product_id': self.product_3.id, 'product_qty': 5}),
            ]
        })
        mo.action_update_bom()
        ...
        self.assertRecordValues(pre_prod_pick.move_ids, [
            {'product_id': self.product_2.id, 'product_qty': 3, 'product_qty_available': 10},
            {'product_id': self.product_3.id, 'product_qty': 5, 'product_qty_available': 20},
        ])
