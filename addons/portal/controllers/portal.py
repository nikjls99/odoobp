...
class CustomerPortal(Controller):
    ...
    def _prepare_address_form_values(
        ...
        if partner_sudo:
            ...
            is_main_address = partner_sudo.type == 'contact'
        else:
        ...
        return {
            ...
            'can_edit_country': not partner_sudo or partner_sudo.can_edit_country(),
            'callback': callback,
    ...
    def _create_or_update_address(
        ...
        **form_data
        ...
        if not partner_sudo:  # Creation of a new address.
            ...
            if not partner_sudo.is_company and 'company_name' in address_values:
                parent_company = partner_sudo.parent_id

                if company_name := address_values['company_name']:
                    if not parent_company:
                        partner_sudo._create_company(company_name)
                    elif parent_company.name != company_name:
                        parent_company.name = company_name
                elif parent_company and parent_company.is_company:
                    partner_sudo.parent_id = False
