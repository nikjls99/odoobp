...
class ResPartner(models.Model):
    ...
    def name_search(self, name='', domain=None, operator='ilike', limit=100):
        ...
        product_sellers = [partner for partner in base_res if partner[0] in unique_sellers]

        return product_sellers + res_minus_sellers[:limit]  # Display product sellers first
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            if product_id:
                ...
                seller_ids = set(product.seller_ids.mapped('partner_id').ids)
                if rec.id in seller_ids:
                    rec.display_name = f'**{rec.name}**'  # Display product sellers in bold
                    continue
