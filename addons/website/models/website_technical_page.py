...
class WebsiteTechnicalPage(models.Model):
    ...
    module_id = fields.Many2one(
        "ir.module.module", string="Related Module")
    ...
    def Import_static_url(self, module_id=None, *args, **kwargs):
        """
        Import static public frontend routes (list_as_website_content=True),
        ...
        for rule in self.env["ir.http"].routing_map().iter_rules():
            if rule.endpoint.routing.get("list_as_website_content"):
