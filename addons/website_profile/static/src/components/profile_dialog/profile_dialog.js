import { Wysiwyg } from "@html_editor/wysiwyg";
import { onMounted, onWillStart, reactive, useRef, useState } from "@odoo/owl";
import { ConfirmationDialog } from "@web/core/confirmation_dialog/confirmation_dialog";
import { localization } from "@web/core/l10n/localization";
import { _t } from "@web/core/l10n/translation";
import { rpc } from "@web/core/network/rpc";
import { user } from "@web/core/user";
import { useAutofocus, useService } from "@web/core/utils/hooks";
import { isEmail } from "@web/core/utils/strings";
import { isHtmlEmpty } from "@web/core/utils/html";
import { uuid } from "@web/views/utils";
import { setCursorEnd } from "@web_editor/js/editor/odoo-editor/src/utils/utils";

export class ProfileDialog extends ConfirmationDialog {
    static template = "website_profile.ProfileDialog";
    static components = {
        ...ConfirmationDialog.components,
        Wysiwyg,
    };
    static props = {
        ...ConfirmationDialog.props,
        focusWebsiteDescription: {
            type: Boolean,
            optional: true,
        },
        userId: { type: Number },
    };
    static defaultProps = {
        ...ConfirmationDialog.defaultProps,
        cancel: () => {},
        cancelLabel: _t("Discard"),
        confirm: () => true,
        confirmLabel: _t("Update"),
        dismiss: () => {},
        focusWebsiteDescription: false,
        title: _t("Edit Profile"),
    };

    setup() {
        super.setup();
        this.orm = useService("orm");
        const componentUUID = uuid();
        this.getUniqueDomID = (suffix) => `profile-dialog-${componentUUID}-${suffix}`;
        this.upload = useRef("upload");
        this.profileImg = useRef("profileImg");
        this.profileImgData = null;
        this.state = useState({ hasError: false, emailHasError: false, nameHasError: false });
        useAutofocus({ refName: "name" });
        const websiteDescriptionClass = this.getUniqueDomID("websiteDescription");
        onWillStart(async () => {
            const fields = [
                "city",
                "country_id",
                "email",
                "name",
                "website",
                "website_description",
                "website_published",
            ];
            const [users, countries] = await Promise.all([
                this.orm.read("res.users", [this.props.userId], fields),
                this.orm.searchRead("res.country", [], ["id", "name"]),
            ]);
            this.user = reactive(users[0], () => this.validate());
            this.user.country_id = this.user.country_id && this.user.country_id[0]; // keep only id
            this.countries = countries;
            this.descriptionConfig = {
                allowImage: false,
                classList: ["form-control", websiteDescriptionClass],
                content: this.user.website_description,
                debug: !!this.env.debug,
                direction: localization.direction || "ltr",
                placeholder: _t("Write a few words about yourself..."),
            };
        });
        onMounted(() => {
            if (this.props.focusWebsiteDescription) {
                setTimeout(() => {
                    const websiteDescription = document.querySelector(
                        `.${websiteDescriptionClass}[contenteditable="true"]`
                    );
                    if (websiteDescription) {
                        setCursorEnd(websiteDescription);
                    }
                }, 50);
            }
            this.validate();
        });
    }

    get websiteDescription() {
        const elContent = this.websiteDescriptionEditor.getElContent();
        if (isHtmlEmpty(elContent.innerText)) {
            return "";
        }
        return elContent.innerHTML;
    }

    get isEditingMyProfile() {
        return user.userId === this.props.userId;
    }

    onEditProfileImg() {
        this.upload.el.click();
    }

    onChangeProfileImg() {
        const reader = new window.FileReader();
        reader.addEventListener("load", (ev) => {
            const result = ev.target.result;
            this.profileImg.el.src = result;
            this.profileImgData = result.toString().replace(/^data:.*;base64,/, "");
        });
        reader.readAsDataURL(this.upload.el.files[0]);
    }

    onClearProfileImg() {
        this.profileImgData = false;
        this.profileImg.el.src = "/web/static/img/placeholder.png";
    }

    onWebsiteDescriptionEditorLoad(editor) {
        this.websiteDescriptionEditor = editor;
    }

    async _confirm() {
        const data = {
            ...this.user,
            country_id: this.user.country_id && parseInt(this.user.country_id),
            website_description: this.websiteDescription,
            user_id: this.props.userId,
        };
        if (this.profileImgData != null) {
            data.image_1920 = this.profileImgData;
        }
        await rpc("/profile/user/save", data);
        return await super._confirm();
    }

    validate() {
        this.state.emailHasError = !isEmail(this.user.email);
        this.state.nameHasError = !this.user.name || !this.user.name.trim().length;
        this.state.hasError = this.state.emailHasError || this.state.nameHasError;
        if (this.modalRef.el) {
            this.modalRef.el.querySelector(".modal-footer button").disabled = this.state.hasError;
        }
    }
}
