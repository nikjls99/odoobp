...
class Domain:
    ...
    def filter_records(self, records: M) -> M:
        ...
        if not records:
            return records
        predicate = self._filter_records(records)
        return records.browse(record_id for record_id, record in zip(records._ids, records) if predicate(record))

    def _filter_records(self, records: M) -> Callable[[M], bool]:
        """Create a predicate for the domain (bound to records)."""
        raise NotImplementedError
...
class DomainBool(Domain):
    ...
    def filter_records(self, records: M) -> M:
        return records if self.value else records.browse()
...
class DomainNot(Domain):
    ...
    def _filter_records(self, records):
        predicate = self.child._filter_records(records)
...
class DomainAnd(DomainNary):
    ...
    def _filter_records(self, records):
        ...
        def filter_and(record):
            if not all(pred(record) for pred in predicates):
                return False
            for index in range(len(predicates), len(self.children)):
                child = self.children[index]
                predicates.append(pred := child._filter_records(records))
                if not pred(record):
                    return False
            return True
...
class DomainOr(DomainNary):
    ...
    def _filter_records(self, records):
        ...
        def filter_or(record):
            if any(pred(record) for pred in predicates):
                return True
            for index in range(len(predicates), len(self.children)):
                child = self.children[index]
                predicates.append(pred := child._filter_records(records))
...
class DomainCondition(Domain):
    ...
    def _filter_records(self, records):
        ...
        if self._opt_level < OptimizationLevel.BASIC:
            return self.optimize(records, full=False)._filter_records(records)
        operator = self.operator
        if operator in ('child_of', 'parent_of'):
            # TODO have a specific implementation for these
            return self.optimize(records, full=True)._filter_records(records)
        ...
        if isinstance(value, Query):
            ...
            return DomainCondition(field_expr, operator, value)._filter_records(records)
